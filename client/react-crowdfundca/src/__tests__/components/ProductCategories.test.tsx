...
describe("ProductCategories component", () => {
  ...
  test("initially displays the correct number of projects", async () => {
    (apiService.fetchData as jest.Mock).mockImplementationOnce(
      (_, __, callback) => {
        callback({ projectInfo: projectInfo });
      }
    );
  ...
  test('loads more projects on "LOAD MORE" button click', async () => {
    ...
    expect(await screen.findAllByText(/Project/i)).toHaveLength(1);
    
    waitFor(() => async () => {
      const loadMorePageButton = await findByText(/LOAD MORE/i);
      expect(loadMorePageButton).toBeInTheDocument();
      fireEvent.click(loadMorePageButton);
      expect(
        require("../../services/apiService").fetchData
      ).toHaveBeenCalledWith(
        "/api/project/showProjectBy?queryKey=categoryid&value=1",
        null,
        expect.any(Function)
      );
      expect(await screen.findAllByText(/Project/i)).toHaveLength(2);
    });
  });
});
